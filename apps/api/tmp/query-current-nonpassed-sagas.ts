...
const latest = db.$with('latest').as(
  ...
  }).from(sagaRuns).groupBy(sagaRuns.sagaKey),
)

const rows = await db
  .with(latest)
  .select({ sagaKey: sagaRuns.sagaKey, id: sagaRuns.id, status: sagaRuns.status, createdAt: sagaRuns.createdAt })
  .from(sagaRuns)
  .innerJoin(latest, and(eq(sagaRuns.sagaKey, latest.sagaKey), eq(sagaRuns.id, latest.maxId)))
  .where(inArray(sagaRuns.status, ['failed', 'running', 'blocked']))
