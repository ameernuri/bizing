...
- [2026-02-12] **Local Embeddings (nomic-embed-text)** — Replaced OpenAI embeddings with local nomic-embed-text (274MB) as primary provider. OpenAI is now fallback. Benefits: 100% private, zero API costs, works offline. Auto-detects Ollama availability and falls back to OpenAI seamlessly. Added `/api/v1/mind/embeddings/status` endpoint to show active provider.
